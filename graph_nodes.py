...
class CRAGGraph:
    ...
    def prepare_context(self, query: str) -> Dict[str, Any]:
        """
        Executes HyDE -> Retrieval -> Context Grading -> (Optional Fallback Search)
        and prepares prompt for streaming response generation.
        """
        state_1 = self.hyde_generator_node({"query": query, "node_trace": []})
        state_2 = self.hybrid_retrieval_node({"query": query, "hyde_doc": state_1["hyde_doc"], "node_trace": state_1["node_trace"]})
        state_3 = self.context_grader_node({"query": query, "documents": state_2["documents"], "node_trace": state_2["node_trace"]})

        if state_3.get("fallback_required", False):
            state_4 = self.fallback_search_node({"query": query, "node_trace": state_3["node_trace"]})
            documents = state_4.get("graded_documents", [])
            source_type = state_4.get("source_type", "Corrected via Tavily Web Search")
            node_trace = state_4.get("node_trace", [])
        else:
            documents = state_3.get("graded_documents", [])
            source_type = "Retrieved from Qdrant Vector Store"
            node_trace = state_3.get("node_trace", [])
        ...
        node_trace.append("✨ [Node 5: Answer Generator] Streaming real-time response tokens...")
        node_trace.append(f"   └─ Stream active. Confidence: {confidence_score * 100:.0f}%. Source: {source_type}")
    ...
    def stream_generation(self, prompt: str):
        """Streams response tokens chunk by chunk from Gemini API."""
        if self.client and config.GEMINI_API_KEY:
            try:
        ...
        # Fallback generator if API key is missing or fails
        fallback_msg = "Synthesized Answer based on context."
